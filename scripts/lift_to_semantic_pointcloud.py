...
def main() -> None:
    ...
    # KITTI odometry ships poses in a *separate* archive; the file is at
    # ``poses/<seq>.txt``, NOT ``sequences/<seq>/poses.txt``. Some mirrors
    # (e.g. TUM i9 ``data_odometry_color/dataset/sequences``) instead drop the
    # KITTI ground-truth file *inside* each sequence dir as ``<seq>/<seq>.txt``
    # while leaving ``poses.txt`` empty or as a broken symlink. To support all
    # layouts transparently we try several candidates and pick the first that
    # both exists and is non-empty.
    dataset_root = Path(args.dataset_root)
    poses_candidates = [
        poses_root / seq / "poses.txt",        # custom: <root>/<seq>/poses.txt
        poses_root / f"{seq}.txt",             # KITTI:  <root>/<seq>.txt   (root = .../poses)
        poses_root / "poses" / f"{seq}.txt",   # KITTI: <root>/poses/<seq>.txt
        dataset_root / seq / f"{seq}.txt",     # i9 mirror: sequences/<seq>/<seq>.txt
    ]
    poses_path = None
    for cand in poses_candidates:
        if cand.is_file() and cand.stat().st_size > 0:
            poses_path = cand
            break
    if poses_path is None:
        # Fall back to the default so the existence check below produces a
        # helpful "not found" message that lists where we actually looked.
        poses_path = poses_candidates[0]
    ...
    for label, path in [
        ...
        if not Path(path).exists():
            ...
            if label == "poses.txt":
                tried = "\n          tried:\n            - " + \
                        "\n            - ".join(str(c) for c in poses_candidates) + \
                        "\n          Hint: KITTI odometry poses live in a separate archive, " \
                        "typically .../data_odometry_poses/dataset/poses/<seq>.txt. " \
                        "Pass --poses_root pointing at that 'poses' directory."
            sys.exit(f"[error] {label} not found: {path}{tried}")
    ...
    if len(all_poses) == 0:
        ...
        if seq == "00":
            seq00_note = (
                "\n        NOTE: on the TUM i9 KITTI mirror, sequence 00 has NO "
                "KITTI ground-truth pose file (only OKVIS '*_trajectory.csv' "
                "outputs in the sequence dir). Pick another sequence (01–10) or "
                "convert one of the OKVIS CSVs to KITTI 3×4 format."
            )
        sys.exit(
            f"[error] 0 poses parsed from {poses_path}. The file exists but is "
            "empty or whitespace-only. On standard KITTI odometry, ground-truth "
            "poses live in a SEPARATE archive (data_odometry_poses); on the i9 "
            "mirror they are dropped inside each sequence as <seq>/<seq>.txt. "
            "Pass --poses_root pointing at the directory that actually contains "
            f"the file, e.g.\n"
            "    --poses_root /storage/group/dataset_mirrors/kitti_odom_color/"
    ...
    if not frame_indices:
        sys.exit(
            f"[error] No frames to process: start_frame={start}, n_frames={args.n_frames}, "
            f"available poses={len(all_poses)}. Check --start_frame / --n_frames."
        )
