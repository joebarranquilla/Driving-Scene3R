...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        default="/usr/prakt/s0043/sam3_predictions",
...
def process_sequence(
    ...
    for fp, result in tqdm(
        ...
        # Save object-only images with transparent background (per-instance
        # and combined). The object pixels keep their original RGB values;
        # background pixels are fully transparent.
        try:
            from PIL import Image as _PIL
            img_rgb = _PIL.open(fp).convert("RGB")
            img_np  = np.array(img_rgb)
            h, w = img_np.shape[:2]

            if inst_seg is not None and inst_seg.sum() > 0:
                # Combined objects (all instances) -> keep any inst_seg > 0
                combined_mask = inst_seg > 0
                alpha = (combined_mask.astype(np.uint8) * 255)
                rgba = np.dstack([img_np, alpha])
                _PIL.fromarray(rgba).save(os.path.join(out_seq_dir, f"{stem}_objects.png"))

                # Individual instance images
                for tid in track_ids:
                    ...
                    rgba = np.dstack([img_np, alpha])
                    _PIL.fromarray(rgba).save(os.path.join(out_seq_dir, f"{stem}_obj_{int(tid)}.png"))
        except Exception:
            # Best-effort: don't fail the whole processing if image write fails
            pass
